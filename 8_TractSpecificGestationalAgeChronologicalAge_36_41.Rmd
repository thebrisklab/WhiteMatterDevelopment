...
minChronAge = function(GAlow,GAhigh=40,model,ylab,less=TRUE,print.plot=FALSE) {
  ...
  a=plot_diff(model,view='chor_age',comp=list(GA=c(GAlow,GAhigh)),sim.ci = TRUE, hide.label=TRUE,xlab='Chronological age (days)',ylab=ylab,plot=print.plot)
  if(less) {
  min(a$chor_age[a$est+a$sim.C2I<0])
  } else {
...
my.function.ga <- function(dti.tract, my.ylim = c(0.1,0.3), my.ylab,  my.main="", estMaxChronAge=TRUE,estMinChronAge=FALSE,print.legend=FALSE,plot.diff=FALSE,less=TRUE) {
    model0 = gam(dti.tract~ti(chor_age,k=9)+ti(GA,k=5)+ti(chor_age,GA,k=c(9,5))+Sex+s(Unique_ID,bs='re')+s(Family_ID,bs='re')+s(Mean_RMS),method='REML',data=dat_harmonized)
 ...
 # gam.check(model0)
  ...
  cColors = viridis::plasma(15)
    ...
    model0.plot = plot_smooth(model0, view="chor_age",cond=list(GA=36),col=cColors[1],rug=FALSE,ylim=my.ylim,lwd=2,se=0,ylab=my.ylab,xlab='Chronological age (days)',hide.label=TRUE,main=my.main,print.summary=FALSE,xlim=c(2,201))
  plot_smooth(model0, view="chor_age",  cond=list(GA=37),col=cColors[5],add=TRUE,rug=FALSE,lwd=2,se=0,print.summary=FALSE,xlim=c(2,201))
  plot_smooth(model0, view="chor_age", se=0, cond=list(GA=38),col=cColors[7],add=TRUE,rug=FALSE,lwd=2,print.summary=FALSE,xlim=c(2,201))
  plot_smooth(model0, view="chor_age", se=0, cond=list(GA=39),col=cColors[9],add=TRUE,rug=FALSE,lwd=2,print.summary=FALSE,xlim=c(2,201))
  plot_smooth(model0, view="chor_age", se=0, cond=list(GA=40),col=cColors[11],add=TRUE,rug=FALSE,lwd=2,print.summary=FALSE,xlim=c(2,201))
  plot_smooth(model0, view="chor_age", se=0, cond=list(GA=41),col=cColors[13],add=TRUE,rug=FALSE,lwd=2,print.summary=FALSE,xlim=c(2,201))
...
```{r}
tiff('../Figures/TractSpecific_GestChronAgeFA_36_41.tiff',width=1080,height=1280,pointsize=18)
...
# have to run these again to get the difference curves due to a plotting environment
# issue when using itsadug with my function:
...
```{r}
tiff('../Figures/TractSpecific_GestChronAgeMD_36_41.tiff',width=1080,height=1280,pointsize=18)

par(mfrow=c(4,3))

fx = my.function.ga(dti.tract = dat_harmonized$Fx_md.combat, my.ylab = "FX MD",less=FALSE,my.ylim = c(0.9,1.5))
...
Check days at which there are significant differences in selected tracts:
```{r}
my.function.ga(dti.tract = dat_harmonized$Ci_md.combat, my.ylab="Ci MD",less=FALSE,my.ylim=c(0.9,1.5),plot.diff = TRUE)

my.function.ga(dti.tract = dat_harmonized$ILF_md.combat, my.ylab="ILF MD",less=FALSE,my.ylim=c(0.9,1.5),plot.diff = TRUE)

uf = my.function.ga(dti.tract = dat_harmonized$UF_md.combat, my.ylab="UF MD",less=FALSE,my.ylim=c(0.9,1.5),plot.diff = TRUE)
...
```{r}
tiff('../Figures/TractSpecific_GestChronAgeAD_36_41.tiff',width=1080,height=1280,pointsize=18)
...
```{r}
tiff('../Figures/TractSpecific_GestChronAgeRD_36_41.tiff',width=1080,height=1280,pointsize=18)
par(mfrow=c(4,3))
...
Check if Fx has increasing differences:


```{r}
my.function.ga(dti.tract = dat_harmonized$Fx_md.combat, my.ylab = "FX RD",less=FALSE,my.ylim = c(0.8,1.4),estMinChronAge = TRUE,estMaxChronAge=FALSE,plot.diff = TRUE)

my.function.ga(dti.tract = dat_harmonized$Fx_ad.combat, my.ylab = "FX RD",less=FALSE,estMinChronAge = TRUE,estMaxChronAge=FALSE,plot.diff = TRUE)

my.function.ga(dti.tract = dat_harmonized$Fx_rd.combat, my.ylab = "FX RD",less=FALSE,estMinChronAge = TRUE,estMaxChronAge=FALSE,plot.diff = TRUE)
