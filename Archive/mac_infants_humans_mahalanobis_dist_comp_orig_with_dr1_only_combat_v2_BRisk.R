...
# TO DO:
# revise the calculation of derivative and percentage change rate variant
...
mac=read.table("dat_harmonized_from_BRisk_11Tracts_old_format.txt"
, header=TRUE)
uniDTI=unique(mac$DTI_type)
uniReg=unique(mac$ROI)
uniReg_len=length(uniReg)

hcp_filename="dti_data_extracted_by_tracts_all_atlas_symmetry_xyflipped_2_fa_mean_adults.txt"
...
for (mah_type in 1:length(unique(df_comb$mah_type)))
{
	...
	for (j in 1:mac_dim[2])
	{
		...
		#m5_gam=gam(mah_dis~ s(Corr_age) + s(Unique_ID), method='REML', data=mahT)
	...
	# BR This does not appear to be used:	
...
pdf(file=paste(base_name,"_mah_corr_bet_orig_and_speed_all.pdf", sep=""), width=15, height=4)
...
pdf(file=paste(base_name,"_mah_corr_bet_orig_and_speed_orig_all.pdf", sep=""), width=15, height=4)
plist<-mget(paste0("p", 1:3))
do.call(grid.arrange, c(plist,nrow=1))
dev.off()
